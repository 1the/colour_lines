...
def win(player):
    messagebox.showinfo(title="Winner Winner Chicken Dinner", message="Winner is player: "+ str(player) + "\nwith score: "+  str(redScore) if player == 1 else str(blueScore))
    wind.destroy()
...
def check():
    ...
    if flag == 16:
        if redScore > blueScore:
            ...
            messagebox.showinfo(title="Hard Luck", message="Tied match, try again")
            wind.destroy()
...
def handle_play(n):
    global player
    n = n-1
    button_list [n]["bg"] = "blue"      # client colour
    player = 1
...
wind.title('Server: Colour Lines')
wind.geometry('450x450')

lb1 = Label(wind, text='player2: RED',fg="red", font=('Helvetica','15'))
lb1.grid(row=0, column=0)

sclbl = Label(wind, text='Score:',fg="red", font=('Helvetica','15'))
sclbl.grid(row=1, column=0)
lbl = Label(wind, fg="red", font=('Helvetica', '15'))
